...
| Desktop Mode | Phase 1 source/package foundation. Primary launchers start VulnoraIQ on the host, create local `scan-reports/` and `agent-lab/` folders, and open a local single-user/admin WebUI session. |
...
| Identity | Local single-user/admin mode for current desktop/lab scope; production token auth and reverse-proxy identity are available for hardened internal deployments. Direct OIDC/JWT is future work. |
